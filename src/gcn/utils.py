...
def load_data_vis(dataset_str, use_trainval):
    """Load data."""
    ...
    val_mask = sample_mask(idx_val, labels.shape[0])
    # test_mask = sample_mask(idx_test, labels.shape[0])
    trainval_mask = sample_mask(idx_trainval, labels.shape[0])
...
def load_data_vis_dense(dataset_str, use_trainval, feat_suffix):
    """Load data."""
    names = [feat_suffix, 'ally', 'graph']
    objects = []
    for i in range(len(names)):
        with open("{}/ind.NELL.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
...
def load_data_vis_multi_sparse(dataset_str, use_trainval, feat_suffix, label_suffix='ally_multi'):
    """Load data."""
    names = [feat_suffix, label_suffix, 'graph']
    objects = []
    for i in range(len(names)):
